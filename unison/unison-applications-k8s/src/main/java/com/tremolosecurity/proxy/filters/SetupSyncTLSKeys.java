...
import com.tremolosecurity.proxy.tls.CheckForKeys;
...
public class SetupSyncTLSKeys implements HttpFilter {
    ...
    public synchronized void initFilter(HttpFilterConfig config) throws Exception {
        ...
        synchronized (semaphore) {
            if (watch == null) {
                this.watch = new SecretVersionsWatch(GlobalEntries.getGlobalEntries().getConfigManager(), GlobalEntries.getGlobalEntries().getConfigManager().getProvisioningEngine(), target, ns);
