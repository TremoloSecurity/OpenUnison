...
public class GroupBase2Attribute implements HttpFilter {

	static Logger logger = Logger.getLogger(GroupBase2Attribute.class.getName());
	...
	public void doFilter(HttpFilterRequest request,
		...
		if (session.getAttribute(key) == null) {
			...
			LDAPSearchResults res = cfgMgr.getMyVD().search(groupDN, 2, equal("uniqueMember",authInfo.getUserDN()).toString(), attribs);
			if (res.hasMore()) {
				res.next();
				isMember = true;
	...
	public void initFilter(HttpFilterConfig config) throws Exception {
		...
		b.append("GROUPBASE2ATTR_").append(this.attributeName).append("_").append(this.attributeValue).append("_").append(this.groupDN).append("_RUN");
		this.key = b.toString();
