...
public class OpenUnisonAttributeMapping implements Insert {
	...
	public void configure(String name, Properties props, NameSpace nameSpace) throws LDAPException {
		...
		ProvisionMappingsType pmst = new ProvisionMappingsType();
		
		int numMappings = Integer.parseInt(props.getProperty("numMappings"));
		for (int i=0;i<numMappings;i++) {
			String config = props.getProperty("map." + i);
			StringTokenizer toker = new StringTokenizer(config,";");
			toker.hasMoreTokens();
			String dest = toker.nextToken();
			toker.hasMoreTokens();
			String src = toker.nextToken();
			toker.hasMoreTokens();
			String mappingType = toker.nextToken();
			
			ProvisionMappingType pmt = new ProvisionMappingType();
			pmt.setTargetAttributeName(dest);
			pmt.setTargetAttributeSource(src);
			pmt.setSourceType(mappingType);
			pmst.getMapping().add(pmt);
		}
		
		try {
			this.mapid = new MapIdentity(pmst);
		} catch (ProvisioningException e) {
			throw new LDAPException(LDAPException.resultCodeToString(LDAPException.OPERATIONS_ERROR),LDAPException.OPERATIONS_ERROR,"Unable to initialize mapping",e);
		}
	...
	public void postSearchEntry(PostSearchEntryInterceptorChain chain, Entry entry, DistinguishedName base, Int scope,
		...
		this.mapid.mapUser(chain, entry, base, scope, filter, attributes, typesOnly, constraints);
	}
