...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.tremolosecurity.proxy.auth.*,java.util.*,com.tremolosecurity.proxy.auth.secret.*,com.tremolosecurity.proxy.util.*,com.tremolosecurity.config.util.*"%>
<!DOCTYPE html>
<html lang="en">
<%
...
ArrayList<SecretQuestion> questions = (ArrayList<SecretQuestion>) request.getAttribute("TREMOLO_SECRET_QUESTIONS");
ArrayList<String> questionList = (ArrayList<String>) request.getAttribute("TREMOLO_SECRET_QUESTION_LIST");
%>
          ...
          <div class="row row-cols-1">
            <div class="col">
              <div class="form-outline mb-4" data-mdb-input-init>
                <input type="text" id="answer<%= i %>" name="answer<%= i %>" class="form-control" />
                <label class="form-label" for="answer<%= i %>" id="placeholder<%= i %>"><%= questionList.get(sq.getQuestion()) %></label>
              </div>
            </div>
          </div>
          <% i++; %>
