...
public class AddDnAttributes implements Insert {
	String name;
	...
	public void postSearchEntry(PostSearchEntryInterceptorChain chain, Entry entry, DistinguishedName base, Int scope,
		...
		boolean addDNAttribute = false;
		
		if (attributes.size() == 0) {
			addDNAttribute = true;
		} else {
			for (Attribute attr : attributes) {
				if (attr.getAttribute().getName().equals("*")) {
					addDNAttribute = true;
				}
			}
		}
		...
		if (addDNAttribute) {
			LDAPAttribute dnAttr = new LDAPAttribute("distinguishedName",entry.getEntry().getDN());
			entry.getEntry().getAttributeSet().add(dnAttr);
		}
